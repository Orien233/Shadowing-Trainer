import type {
  ...
  AIProvider, ASRSceneSettings, ASRSceneSettingsUpdate, ProviderCatalogEntry, ProviderTestResponse,
...
export const testProvider = (id: number, payload: Partial<ProviderInput> = {}) => request<ProviderTestResponse>(`/api/providers/${id}/test`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }, 130_000);
export const testProviderDraft = (payload: ProviderInput) => request<ProviderTestResponse>("/api/providers/test", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }, 130_000);
