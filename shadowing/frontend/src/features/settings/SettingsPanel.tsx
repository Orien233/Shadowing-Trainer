...
export default function SettingsPanel({ onProvidersChanged }: { onProvidersChanged?: () => void }) {
  ...
  return <div className="card settings-panel"><p className="muted">{t("settings.credentialsHint")}</p>
    <section className="provider-section"><h3>{t("settings.quickTemplates")}</h3><p className="muted">{t("settings.templatesHint")}</p><div className="panel-actions template-actions">{catalog.filter((entry) => entry.preset !== false).map((entry) => { const selected = catalogId(entry) === catalogId(selectedCatalog); return <button key={catalogId(entry)} type="button" className={`template-button ${selected ? "active" : ""}`} aria-pressed={selected} onClick={() => { setDraft(newDraft(entry)); setEditingProviderId(null); setDraftTest(null); }}>{adapterLabel(entry)}</button>; })}</div></section>
